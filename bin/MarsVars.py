...
parser = argparse.ArgumentParser(description = '''\033[93m MarsVars.py, variable manager, utility to add or '''\
    '''remove variables to the diagnostic files\n\n Use MarsFiles ****.atmos.average.nc to view file content. \033[00m''',
...
parser.add_argument('-add','--add', nargs = '+',default = [],
                 help = 'Add a new variable to file. \n'
                      '> Usage: MarsVars.py ****.atmos.average.nc -add rho \n'
                      ...
                      '     and zfull are added before interpolating to pstd, zagl, or zstd. \n'
                      '\033[00m\n' 
...
parser.add_argument('-zdiff','--zdiff', nargs = '+',default = [],
                 help = 'Differentiate a variable with respect to the z axis. \n'
                        'A new a variable (d_dz_var) in [Unit/m] will be added to the file. \n'
                        '> Usage: MarsVars.py ****.atmos.average.nc -zdiff temp \n')
...
parser.add_argument('-col','--col', nargs = '+',default = [],
                 help = 'Integrate a mixing ratio of a variable through the column \n'
                        ...
                        '> Usage: MarsVars.py ****.atmos.average.nc -col ice_mass \n')
                             
parser.add_argument('-zd','--zonal_detrend', nargs = '+',default = [],
                 help = 'Detrend a variable by substracting its zonal mean value. \n'
                        'A new a variable (var_p, "p" for prime) will be added to the file. \n'
                        '> Usage: MarsVars.py ****.atmos.average.nc -zd ucomp \n')       
                      
parser.add_argument('-rm','--remove', nargs = '+',default = [],
                 help = 'Remove a variable from the file. \n'
                        '> Usage: MarsVars.py ****.atmos.average.nc -rm rho theta \n')
                      
parser.add_argument('-extract','--extract', nargs = '+',default = [],
                 help = 'Extract a variable(s) to a new file (_extract.nc). \n'
                        '> Usage: MarsVars.py ****.atmos.average.nc -extract ps ts \n')

parser.add_argument('--debug',  action = 'store_true', help = 'Debug flag: release the exceptions')


# =====================================================================
# This is the list of supported variables for the add action: 
...
# Fill values for NaN. Do not use np.NaN, this raises an issue when running runpinterp.
fill_value=0.
...
Cp          = 735.0     # J/K
M_co2       = 0.044     # kg/mol
N           = 0.01      # rad/s,            for Ep calculation
...
def compute_theta(p_3D, ps, temp, f_type):
    ...
    ps_shape = ps.shape
    if f_type == 'diurn':
        ps_shape = [ps_shape[0], ps_shape[1], 1, ps_shape[2], ps_shape[3]]    # (time, tod, lat, lon) -> (time, tod, 1, lat, lon)
    else:   
        ps_shape = [ps_shape[0], 1, ps_shape[1], ps_shape[2]]    # (time, lat, lon) -> (time, 1, lat, lon)
...
def compute_w(rho, omega):
    '''
    Compute atmospheric density in kg/m3
    ''' 
    return -omega/(rho*g)
...
def compute_DZ_full_pstd(pstd,temp,ftype='average'):
    '''
    Return the distance between two layers. Mid-point from the standard pressure level.
    ...
    Returns:
        DZ_full_pstd: 3D array of distances between adjacent layers.
        
    *** NOTE***
    In this context, pfull = pstd, with the half-layer boundaries defined somewhere between successive layers.
    ...
    reshape_shape[0] = len(pstd)  # eg (28, 1, 1, 1)
    ...
    there is typically data missing from last few layers so this is not a big issue.
    '''
...
def compute_scorer(N, ucomp, zfull):
    '''
    Compute the Scorer wavelength in [m]
    '''
...
def compute_WMFF(MF, rho, lev, interp_type):
    ...
    ***NOTE***                                            ay = -1/rho d(rho v'w')/dz in [m/s/s]
    ...
    if interp_type == 'pstd':
        # After computing du/dp above, multiply by (-rho * g) to obtain du/dz.
        return g* darr_dz
    else:   # if zagl and zstd, levels are already in meters so we effectively have du/dz already.
...
def main():
    ...
    global lev_T        # Array to swap first and second vertical axes: (1, 0, 2, 3) for (time, lev, lat, lon); (2, 1, 0, 3, 4) for (tim, tod, lev, lat, lon).
    global lev_T_out    # Reshape array in zfull, zhalf calculation.
    
    # Check if an operation is requested, otherwise print the file content.
    ...
    for ifile in file_list:
        ...
        # =================================================================
        #                       Remove action 
        #           MarsVars.py FILE -rm (--remove) VAR
        # =================================================================
        ...
        # =================================================================
        #                       Add action 
        #               MarsVars.py FILE -add VAR
        # =================================================================
        
        # If the list is not empty, load ak and bk for pressure calculation.
        if add_list: 
        ...
        #----    
            ...
            #---- Check if the variable is currently supported by MarsVars ---
        for ivar in add_list:
            ...
            if ivar not in VAR.keys():
                ...
                try:
                    ...
                    if f_type == 'diurn':
                        lev_T = [2, 1, 0, 3, 4] # (tim, tod, lev, lat, lon) -> (lev, tod, tim, lat, lon) -> (tim, tod, lev, lat, lon)
                        #                      0    1    2    3    4        2    1    0    3    4        2    1    0    3    4 
                        lev_T_out = [1,2,0,3,4]
                        lev_axis  = 2 # in atmos_diurn the level is the 3rd axis: (tim, tod, lev, lat, lon)
                    else:
                        lev_T = [1,0,2,3] # (tim, lev, lat, lon) -> [lev, tim, lat, lon) -> (tim, lev, lat, lon)
                        #                    0    1    2    3        1    0    2    3        1    0    2    3
                        lev_T_out = lev_T
                        lev_axis  = 1 # in atmos_average, atmos_daily, the levels is the 2nd axis' (tim,lev, lat, lon)
                            
                    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     
                    #~~~~~~~~~~~~  Non interpolated files ~~~~~~~~~~~~~~~~~
                    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                     
                    # These are often needed so extract them once here
                    ...
                    # If interp_type is 'zstd', or 'zagl', we need the pfull3D field before interpolating.
                    # Use a 'try' statement as some computations (e.g. wind speed) do not require pfull3D and will work anyway.
                    else:
                        try:
                    ...
                    if ivar == 'zfull': OUT = compute_zfull(ps, ak, bk, temp)  # TODO not with pstd
                    ...
                    # Add nan for interpolated files
                    else:
        ...
        #                   MarsVars.py FILE -zdiff VAR
        # =================================================================
        
        # ak and bk are needed to derive the distance between layers (pfull)
        if zdiff_list: 
            name_fixed    = find_fixedfile(None, ifile)
            f_fixed       = Dataset(name_fixed, 'r', format = 'NETCDF4_CLASSIC')
            variableNames = f_fixed.variables.keys();
        ...
        # =================================================================
        #                       Column Integration Action
        #               MarsVars.py FILE -col VAR
        # =================================================================
        '''
                          z_top
                          ⌠
            ...
            if icol not in fileNC.variables.keys():
                ...
                try:
                    ...
                    if f_type == 'diurn':
                        ...
                        lev_axis = 2  # in atmos_diurn the level is the 3rd axis: (time, tod, lev, lat, lon)
                    else:   # (time, lat, lon) 
                        lev_T    = [1,0,2,3]    # (tim, lev, lat, lon)
